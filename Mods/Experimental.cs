...
namespace iiMenu.Mods
{
    public static class Experimental
    {
        ...
        private static IEnumerator FreezeCoroutine(VRRig rig)
        {
            ...
            while (GorillaParent.instance.GetRigs().Contains(rig))
            {
        ...
        public static void AdminStrangle()
        {
            if (leftGrab)
            {
                ...
                {
                    foreach (var rig in GorillaParent.instance.GetRigs().Where(rig => !rig.isLocal).Where(rig => Vector3.Distance(rig.headMesh.transform.position, GorillaTagger.Instance.leftHandTransform.position) < 0.2f))
                    {
            ...
            if (rightGrab)
            {
                ...
                {
                    foreach (var rig in GorillaParent.instance.GetRigs().Where(rig => !rig.isLocal).Where(rig => Vector3.Distance(rig.headMesh.transform.position, GorillaTagger.Instance.rightHandTransform.position) < 0.2f))
                    {
                        thestrangled = rig;
                        if (PhotonNetwork.InRoom)
