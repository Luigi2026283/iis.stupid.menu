    ...
    public static class Safety
    {
        ...
        public static void AntiReport(Action<VRRig, Vector3> onReport)
        {
            ...
            foreach (GorillaPlayerScoreboardLine line in GorillaScoreboardTotalUpdater.allScoreboardLines)
            {
                ...
                foreach (var vrrig in from vrrig in GorillaParent.instance.GetRigs() where !vrrig.isLocal where OverlappingButton(vrrig, report.position) || (antiMute && OverlappingButton(vrrig, line.muteButton.gameObject.transform.position)) where !smartAntiReport || SmartAntiReport(line.linePlayer) select vrrig)
        ...
        {
            foreach (var vrrig in GorillaParent.instance.GetRigs().Where(vrrig => !vrrig.isOfflineVRRig && vrrig.rawCosmeticString.Contains("LBAAK") || vrrig.rawCosmeticString.Contains("LBAAD") || vrrig.rawCosmeticString.Contains("LMAPY")))
            {
                try
                {
        ...
        {
            foreach (var vrrig in GorillaParent.instance.GetRigs().Where(vrrig => !vrrig.isOfflineVRRig && Visuals.specialCosmetics.Keys.Any(x => vrrig.rawCosmeticString.Contains(x))))
            {
                try
                {
        ...
        public static void ColorSpoof()
        {
            ...
            foreach (var rig in GorillaParent.instance.GetRigs().Where(rig => !rig.isLocal).Where(rig => !colorSpoofRigs.Contains(rig)))
            {
                GorillaTagger.Instance.myVRRig.SendRPC("RPC_InitializeNoobMaterial", GetPlayerFromVRRig(rig), Random.Range(0f, 1f), Random.Range(0f, 1f), Random.Range(0f, 1f));
